...
// Controlador global para la calidad del audio
...
// -----------------------------------------------------------------------------
// EL LOGO CONSPIRANOICO: "El Ojo de Osiris" (Frecuencia 432)
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// EL MOTOR DE AUDIO (EL CÓDIGO EXITOSO V1.0)
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// ARQUITECTURA PRINCIPAL DE LA APP (SUPER APP SKELETON)
// -----------------------------------------------------------------------------
...
class _SuperAppSkeletonState extends State<SuperAppSkeleton> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      // IndexedStack preserva el estado (las búsquedas) al cambiar de pestaña
      body: Column(
        children: [
          Expanded(
            child: IndexedStack(
          ...
          // El MiniPlayer ahora es global y flota por encima de los Tabs
          const MiniPlayer(), 
        ],
      ),
    ...
    return Scaffold(
      body: Center(
        child: Column(
          ...
          children: [
            const ConspiracyLogo(size: 180), 
            const SizedBox(height: 40),
            const Text(
              'EL OJO DE OSIRIS',
              style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, letterSpacing: 8, color: Colors.white),
            ),
            const SizedBox(height: 10),
            Text(
              'Frecuencia 432Hz Activada',
              style: TextStyle(fontSize: 14, color: Colors.deepPurpleAccent.withOpacity(0.8), letterSpacing: 2),
            ),
          ],
        ),
      ),
...
// --- TAB 1: BUSCADOR (EL CÓDIGO FUNCIONAL COMPLETO) ---
...
class _SearchScreenState extends State<SearchScreen> {
  ...
  void playVideo(Video video) async {
    ...
    final queueList = videos.map((v) => MediaItem(
      id: v.id.value, title: v.title, artist: v.author, duration: v.duration, artUri: Uri.parse(v.thumbnails.highResUrl),
    )).toList();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Buscador VIP'), backgroundColor: const Color(0xFF1A1A1A), elevation: 0, centerTitle: true),
      body: Column(
        children: [
          Padding(
              ...
              children: ['Todos', 'Podcasts', 'Música de Moda', 'Rock 90s', 'Electrónica'].map((filter) {
                ...
                return Padding(
                  ...
                  child: ChoiceChip(
                    ...
                    selected: isSelected, selectedColor: Colors.deepPurpleAccent, backgroundColor: const Color(0xFF2A2A2A),
          ...
          if (isLoading) const Expanded(child: Center(child: CircularProgressIndicator(color: Colors.deepPurpleAccent)))
          else Expanded(
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  ...
                  trailing: Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      if (isPlaying) const Icon(Icons.equalizer, color: Colors.deepPurpleAccent, size: 24),
    ...
    return const Scaffold(
      body: Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(Icons.lock_outline, size: 80, color: Colors.grey), SizedBox(height: 20),
            Text('Tu Bóveda Privada', style: TextStyle(color: Colors.white, fontSize: 18)),
            Text('Historial y Favoritos vivirán aquí', style: TextStyle(color: Colors.grey)),
          ],
        ),
      ),
    );
    ...
    return Scaffold(
      ...
      body: Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            const Icon(Icons.sports_soccer, size: 80, color: Colors.greenAccent), const SizedBox(height: 20),
            const Text('Tablero VIP Deportivo', style: TextStyle(color: Colors.white, fontSize: 18)),
            Text('Tus equipos bajo vigilancia estricta', style: TextStyle(color: Colors.greenAccent)),
          ],
        ),
      ),
    );
              ...
              children: [
                ...
                StreamBuilder<PlaybackState>(
                  ...
                  builder: (context, snapshot) {
                    ...
                    if (isBuffering) return const Padding(padding: EdgeInsets.all(12.0), child: SizedBox(width: 24, height: 24, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.deepPurpleAccent)));
                    return IconButton(icon: Icon(playing ? Icons.pause_circle_filled : Icons.play_circle_fill), iconSize: 42, color: Colors.deepPurpleAccent, onPressed: () => playing ? audioHandler.pause() : audioHandler.play());
                  }
                ),
              ],
            ),
          ...
          return Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 40),
                Align(alignment: Alignment.centerLeft, child: Text(mediaItem.title, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: Colors.white))),
                const SizedBox(height: 8),
                Align(alignment: Alignment.centerLeft, child: Text(mediaItem.artist ?? '', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 18, color: Colors.grey))),
                ...
                StreamBuilder<Duration>(
                  ...
                  builder: (context, snapshot) {
                    ...
                    return Column(
                      children: [
                        SliderTheme(
                          data: SliderTheme.of(context).copyWith(trackHeight: 4, thumbShape: const RoundSliderThumbShape(enabledThumbRadius: 8), overlayShape: const RoundSliderOverlayShape(overlayRadius: 16), activeTrackColor: Colors.deepPurpleAccent, inactiveTrackColor: Colors.grey[800], thumbColor: Colors.deepPurpleAccent),
                      ...
                      children: [
                        ValueListenableBuilder<bool>(
                          ...
                          builder: (context, isHD, _) {
                            return IconButton(
                              icon: Icon(isHD ? Icons.high_quality : Icons.data_saver_on), color: isHD ? Colors.deepPurpleAccent : Colors.grey,
                              onPressed: () {
                                isHDMode.value = !isHD;
                                ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(isHD ? "Ahorro de datos activado" : "Audio HD Activado"), backgroundColor: isHD ? Colors.orange : Colors.deepPurpleAccent));
                              },
                            );
                          }
                        ),
                        ...
                        Container(
                          width: 80, height: 80, decoration: const BoxDecoration(shape: BoxShape.circle, color: Colors.deepPurpleAccent),
                        ...
                        IconButton(icon: const Icon(Icons.shuffle), color: Colors.grey, onPressed: () { audioHandler.shuffleQueue(); ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Lista mezclada"), backgroundColor: Colors.deepPurpleAccent)); }),
                      ],
                    );
                  }
                ),
